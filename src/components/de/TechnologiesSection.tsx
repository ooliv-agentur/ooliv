        ...
        <div className="mb-12">
          ...
          <StaggerReveal className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {technologies.map((tech, index) => (
              <div key={index} className="bg-medico-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border-2 border-transparent hover:border-medico-turquoise group h-full flex flex-col">
                <div className="flex justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                  {tech.icon}
                </div>
                <h3 className="text-xl font-bold text-center mb-4 text-medico-darkGreen">{tech.name}</h3>
                <p className="text-medico-darkGreen/80 text-center leading-relaxed flex-grow">{tech.description}</p>
              </div>
            ))}
          </StaggerReveal>
        </div>
