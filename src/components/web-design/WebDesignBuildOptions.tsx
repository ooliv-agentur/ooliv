...
const WebDesignBuildOptions = () => {
  ...
  const translations = {
    ...
    de: {
      ...
      subtitle: "Egal ob kompakte Business-Website, Unternehmenswebsite oder E-Commerce – wir entwickeln das passende Design für Ihre Anforderungen. Auf dieser Basis realisieren wir später Ihre Website mit WordPress, Shopify oder Custom-Entwicklung. Mehr zur technischen Umsetzung erfahren Sie auf unserer Seite ",
          ...
          {t.options.map((option, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100 h-full flex flex-col">
              <div className="flex flex-col items-center text-center h-full">
                ...
                <h3 className="text-lg font-bold mb-3 text-brand-heading">{option.title}</h3>
                <p className="text-brand-text text-sm leading-relaxed flex-grow">{option.description}</p>
              </div>
            </div>
          ))}
        </StaggerReveal>
